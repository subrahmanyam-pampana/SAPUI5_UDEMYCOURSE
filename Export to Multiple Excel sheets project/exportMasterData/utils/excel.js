...
function excel() {
    ...
    var oStyles = {
      add: function (a) {
        ...
        if (a.fill && a.fill.charAt(0) == "#")
          style.fill =
            2 + findOrAdd(fills, a.fill.toString().substring(1).toUpperCase()); // If there is a fill color add it, with a gap of 2, because of the TWO DEFAULT HARDCODED fills
        if (a.font)
  ...
  $JExcel.new = function (defaultFont) {
    ...
    excel.set = function (s, column, row, value, style, colspan) {
      if (isObject(s))
        return this.set(s.sheet, s.column, s.row, s.value, s.style, colspan); // If using Object form, expand it
