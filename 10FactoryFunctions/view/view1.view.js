sap.ui.jsview("myApp.view.view1", {
  ...
  createContent: (oController) => {
    ...
    oPageContent.bindAggregation("tiles","/Students",

              function(sId,oContext){
                var sTitle = oContext.getProperty("title");
                var sinfo = oContext.getProperty("info");
                //debugger;
                if(sTitle === "subrahmanyam"){
                  return new sap.m.StandardTile({
                    title:sTitle,
                    icon:"{icon}",
                    info:"He is very Good guy",
                    activeIcon:"sap-icon://display"
                  })
                }else{
                  return new sap.m.StandardTile({
                    title:sTitle,
